#!/usr/bin/env bash
# Run the runplz test suite with a memory- and CPU-aware pytest-xdist
...
if "$PYTHON_BIN" -c "import xdist" 2>/dev/null; then
    ...
    log "workers=${WORKERS} → exec ${PYTHON_BIN} -m pytest -n ${WORKERS} --cov=runplz --cov-report=term-missing tests $*"
else
    log "platform=${OS} cpus=${CPUS} (pytest-xdist not installed; running serial)"
    log "→ exec ${PYTHON_BIN} -m pytest --cov=runplz --cov-report=term-missing tests $*"
fi

exec "$PYTHON_BIN" -m pytest "${XDIST_FLAGS[@]}" --cov=runplz --cov-report=term-missing tests "$@"
