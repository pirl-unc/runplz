...
2. **Every PR bumps the version** in `runplz/version.py`. Even doc-only PRs — at minimum a patch bump.
3. **"Done" means merged AND deployed to PyPI** — never stop at merge. After a PR merges, run `./deploy.sh` from a clean main (builds, uploads via twine, tags + pushes the tag). Skipping deploy = task not done.
