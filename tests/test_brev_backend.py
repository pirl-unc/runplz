...
def test_render_ops_script_run_commands_and_index_url(tmp_path):
    ...
    s = ssh_common.render_image_ops_script(img)
...
def test_wait_until_ssh_reachable_returns_on_first_success(monkeypatch):
    ...
    monkeypatch.setattr(ssh_common, "SSH_OPTS", [])
    calls = []
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_run):
        ssh_common.wait_until_ssh_reachable("box", max_wait_s=60, probe_interval_s=1)
...
def test_wait_until_ssh_reachable_raises_after_deadline(monkeypatch):
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_run):
        with pytest.raises(RuntimeError, match="never became reachable"):
            ssh_common.wait_until_ssh_reachable("box", max_wait_s=5, probe_interval_s=1)
...
def test_wait_until_ssh_reachable_invokes_refresh_callback_periodically(monkeypatch):
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_run):
        with pytest.raises(RuntimeError):
            ssh_common.wait_until_ssh_reachable(
...
def test_ensure_docker_happy_path():
    ...
    with mock.patch(
        ...
        ssh_common.ensure_docker("some-box")  # no raise
...
def test_ensure_docker_falls_back_to_get_docker_sh(capsys):
    ...
    with mock.patch(
        ...
        with mock.patch("runplz.backends.ssh_common.run_local", fake_sh):
            ssh_common.ensure_docker("some-box")
...
def test_remote_has_nvidia_true():
    with mock.patch(
        ...
        assert ssh_common.remote_has_nvidia("box") is True
...
def test_remote_has_nvidia_false():
    with mock.patch(
        ...
        assert ssh_common.remote_has_nvidia("box") is False
...
def test_rsync_up_excludes_and_no_delete(tmp_path):
    ...
    with mock.patch("runplz.backends.ssh_common.run_local", lambda c: recorded.setdefault("c", c)):
        ssh_common.rsync_up(tmp_path, "my-box")
    cmd = recorded["c"]
    assert cmd[:2] == ["rsync", "-az"]
    assert "--delete" not in cmd  # regression: --delete was removed
    assert "--files-from=-" not in cmd  # non-Git directories keep full-tree fallback
...
def test_rsync_up_excludes_configured_outputs_dir(tmp_path):
    ...
    recorded = {}
    with mock.patch("runplz.backends.ssh_common.run_local", lambda c: recorded.setdefault("c", c)):
        ssh_common.rsync_up(tmp_path, "my-box", outputs_dir="brev_runs")
    cmd = recorded["c"]
...
def test_rsync_up_skips_outputs_dir_exclude_for_default(tmp_path):
    ...
    recorded = {}
    with mock.patch("runplz.backends.ssh_common.run_local", lambda c: recorded.setdefault("c", c)):
        ssh_common.rsync_up(tmp_path, "my-box", outputs_dir="out")
    cmd = recorded["c"]
...
def test_rsync_up_handles_nested_outputs_dir(tmp_path):
    ...
    recorded = {}
    with mock.patch("runplz.backends.ssh_common.run_local", lambda c: recorded.setdefault("c", c)):
        ssh_common.rsync_up(tmp_path, "my-box", outputs_dir="data/runs")
    cmd = recorded["c"]
...
def test_rsync_up_skips_outputs_dir_exclude_when_outside_repo(tmp_path):
    ...
    recorded = {}
    with mock.patch("runplz.backends.ssh_common.run_local", lambda c: recorded.setdefault("c", c)):
        ssh_common.rsync_up(tmp_path, "my-box", outputs_dir=str(outside))
    cmd = recorded["c"]
...
def test_prepare_remote_run_writes_manifest_and_latest_link(tmp_path):
    ...
    with mock.patch(
        ...
        ssh_common.prepare_remote_run("box", remote_run, manifest=manifest)
...
def test_rsync_up_uses_remote_run_repo_when_provided(tmp_path):
    ...
    with mock.patch("runplz.backends.ssh_common._record_remote_event", lambda *a, **k: None):
        with mock.patch(
            ...
            ssh_common.rsync_up(tmp_path, "my-box", remote_run=remote_run)
...
def test_run_native_with_remote_run_uses_per_run_lifecycle_files(tmp_path):
    ...
    def fake_sub_run(cmd, *a, **kw):
        # Report that startup was observed and the fast command has already
        # exited, then return exit_code 0 from the lifecycle log.
        cmd_str = " ".join(cmd)
        if "kill -0" in cmd_str:
            return mock.Mock(returncode=0, stdout="1 dead 123", stderr="")
    ...
    # And the portable detach plumbing: nohup + pid file, without setsid's
    # possible wrapper fork.
...
def test_ssh_packages_cmd_in_bash_lc():
    recorded = {}
    with mock.patch("runplz.backends.ssh_common.run_local", lambda c: recorded.setdefault("c", c)):
        ssh_common.ssh_exec("instance", "echo hi")
...
def test_sh_prints_and_runs(capsys):
    with mock.patch(
        ...
        ssh_common.run_local(["echo", "hello world"])
...
def test_ssh_capture_runs_with_capture():
    with mock.patch(
        ...
        out = ssh_common.ssh_capture("box", "echo hi")
...
def test_ensure_remote_rsync_sends_expected_cmd():
    ...
    with mock.patch(
        ...
        ssh_common.ensure_remote_rsync("box")
...
def test_build_image_dockerfile_uses_docker_build_f(tmp_path):
    ...
    with mock.patch(
        ...
        ssh_common.build_image("box", img)
...
# -- stream_and_wait + container_running -------------------------------
...
def test_stream_and_wait_exits_when_container_done():
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_run):
        rc = ssh_common.stream_and_wait("box", "c")
    assert rc == 0
...
def test_stream_and_wait_reconnects_then_gives_up(capsys):
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_run):
        with mock.patch("runplz.backends.ssh_common.time.sleep", lambda _s: None):
            rc = ssh_common.stream_and_wait("box", "c", max_reconnects=3)
    assert rc == 137
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_run):
        with mock.patch("runplz.backends.ssh_common.time.sleep", lambda _s: None):
            with pytest.raises(RuntimeError, match="max_runtime_seconds=7"):
                ssh_common.stream_and_wait("box", "c", max_reconnects=0, max_runtime_seconds=7)
...
def test_container_running_treats_ssh_failure_as_still_running():
    ...
    with mock.patch(
        ...
        assert ssh_common.container_running("box", "c") is True
...
def test_container_running_returns_false_on_inspect_false():
    with mock.patch(
        ...
        assert ssh_common.container_running("box", "c") is False
...
def test_run_vm_docker_mode_nonzero_exit_raises(tmp_path):
    ...
    def fake_ssh_capture(instance, cmd, **kw):
        # The fetch_failure_tail helper issues `docker logs --tail N ...`
    ...
    with (
        ...
        mock.patch.multiple(
            ...
            build_image=mock.DEFAULT,
            run_container_detached=mock.DEFAULT,
            stream_and_wait=mock.Mock(return_value=137),
            ssh_capture=fake_ssh_capture,
            rsync_down=mock.DEFAULT,
        ),
    ):
        # fetch_failure_tail lives in ssh_common and calls ssh_capture
...
def test_run_container_mode_nonzero_exit_includes_remote_log_tail(tmp_path):
    ...
    fetch_failure_tail tails that file when no docker container exists."""
    ...
    with (
        ...
        mock.patch.multiple(
            ...
            ensure_remote_rsync=mock.DEFAULT,
            rsync_up=mock.DEFAULT,
            run_container_mode=mock.Mock(return_value=1),
            ssh_capture=fake_ssh_capture,
            rsync_down=mock.DEFAULT,
        ),
    ):
...
def test_setup_failure_after_create_still_runs_on_finish(tmp_path):
    ...
    with (
        mock.patch.multiple(
            ...
            _refresh_ssh=mock.Mock(
                side_effect=RuntimeError("rpc error: context deadline exceeded")
            ),
            _apply_on_finish=capture_finish,
        ),
        mock.patch.multiple(
...
def test_fetch_failure_tail_returns_empty_on_ssh_error():
    """fetch_failure_tail must never raise — the caller is already about
    ...
    with mock.patch("runplz.backends.ssh_common.ssh_capture", boom):
        out = ssh_common.fetch_failure_tail(target="box", container_name=None)
...
def test_raise_for_runtime_cap_kills_container_and_raises():
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_sub):
        with pytest.raises(RuntimeError) as ei:
            ssh_common.raise_for_runtime_cap("box", 60, container_name="c-abc")
...
def test_raise_for_runtime_cap_pkills_bootstrap_in_native_or_container_mode():
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_sub):
        with pytest.raises(RuntimeError):
            ssh_common.raise_for_runtime_cap("box", 30, container_name=None)
...
def test_stream_and_wait_raises_when_cap_exceeded():
    """stream_and_wait should trip the cap when docker logs -f keeps
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_sub):
        with pytest.raises(RuntimeError, match="max_runtime_seconds=2"):
            ssh_common.stream_and_wait("box", "container-xyz", max_runtime_seconds=2)
...
def test_launch_detached_and_wait_falls_back_to_blocking_without_remote_run():
    ...
    from runplz.backends.ssh_common import launch_detached_and_wait
...
def test_build_detached_launcher_stays_pid_stable_and_avoids_setsid():
    ...
    cmd = build_detached_launcher(remote_run, "echo hi")
...
def test_tail_and_wait_reconnects_when_pid_still_alive():
    ...
    from runplz.backends.ssh_common import tail_and_wait_for_detached
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_sub):
        with mock.patch("runplz.backends.ssh_common.time.sleep"):
            rc = tail_and_wait_for_detached(
                target="box",
                pid_file="/tmp/pid",
                log_file="/tmp/log",
...
def test_read_remote_exit_code_parses_last_entry():
    ...
    from runplz.backends.ssh_common import read_remote_exit_code
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_sub):
        assert read_remote_exit_code("box", "/tmp/events") == 13
...
def test_read_remote_exit_code_defaults_to_1_on_missing_or_malformed():
    ...
    with mock.patch(
        ...
        assert read_remote_exit_code("box", "/tmp/events") == 1
...
def test_remote_pid_alive_treats_ssh_timeout_as_alive():
    ...
    from runplz.backends.ssh_common import remote_pid_alive

    def fake_sub(cmd, *a, **kw):
        raise subprocess.TimeoutExpired(cmd=cmd, timeout=kw.get("timeout", 0))

    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_sub):
        assert remote_pid_alive("box", "/tmp/pid") is True
...
def test_stream_and_wait_no_cap_passes_none_timeout():
    ...
    with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_sub):
        exit_code = ssh_common.stream_and_wait("box", "c-1", max_runtime_seconds=None)
...
def test_run_container_mode_end_to_end(tmp_path):
    ...
    with (
        ...
        mock.patch.multiple(
            ...
            ensure_remote_rsync=mock.DEFAULT,
            rsync_up=mock.DEFAULT,
            run_container_mode=mock.Mock(return_value=0),
            rsync_down=mock.DEFAULT,
        ),
    ):
        brev.run(app, fn, [], {}, instance="box")
...
def _full_run_mocks():
    """Brev-scoped mock.multiple kwargs for brev.run() end-to-end.

    Split from `_full_dispatch_mocks` because brev's dispatch runs through
    `ssh_common.dispatch_to_target` — the provisioning helpers still live on
    the brev module, the staging/run helpers are reached via ssh_common.
    Use both together via `_full_run_patches()`.
    """
    return dict(
        _require_brev_cli=mock.DEFAULT,
        _skip_onboarding=mock.DEFAULT,
...
def _full_dispatch_mocks(**overrides):
    """ssh_common-scoped kwargs for a clean VM-docker-mode dispatch.

    Callers override `stream_and_wait` to control the exit code.
    """
    mocks = dict(
        wait_until_ssh_reachable=mock.DEFAULT,
        prepare_remote_run=mock.DEFAULT,
...
def _full_run_patches(**dispatch_overrides):
    """Patch both halves of a brev.run() end-to-end path at once."""
    with mock.patch.multiple("runplz.backends.brev", **_full_run_mocks()):
        with mock.patch.multiple(
            "runplz.backends.ssh_common", **_full_dispatch_mocks(**dispatch_overrides)
        ):
            yield
...
def test_container_rm_force_called_in_finally_on_failure(tmp_path):
    """Orphaned-container guard: a failing `stream_and_wait` used to leak the
...
def test_run_container_mode_builds_expected_remote_cmd(tmp_path):
    ...
    with mock.patch("runplz.backends.ssh_common.ssh_exec", fake_ssh):
        with mock.patch("runplz.backends.ssh_common.subprocess.run", fake_sub_run):
            rc = ssh_common.run_container_mode(
                target="box",
                function=fn,
                rel_script="jobs/train.py",
                args=[],
...
# ---------------------------------------------------------------------------
# brev runs on the shared dispatch core
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"mode": "vm", "use_docker": True}, "docker"),
        ({"mode": "vm", "use_docker": False}, "native"),
        # mode="container" + use_docker=False is rejected by BrevConfig
        # itself, so there is no fourth case to map.
    ],
)
def test_dispatch_mode_maps_brev_config_onto_the_shared_runner(cfg_kwargs, expect):
    assert brev.dispatch_mode(BrevConfig(**cfg_kwargs)) == expect


def test_brev_run_goes_through_the_shared_dispatch(tmp_path):
    """brev must not keep a private copy of the dispatch sequence."""
    cfg = BrevConfig(mode="vm", use_docker=True, on_finish="leave")
    app = _app(tmp_path, cfg=cfg)
    fn = _function(app, Image.from_registry("ubuntu:22.04"), module_file=_job_inside(tmp_path))
    ...
    with mock.patch.multiple(
        ...
        with mock.patch.object(brev, "run_on_provisioned_vm", return_value=None) as shared_mock:
            brev.run(app, fn, [], {}, instance="box")

    kwargs = shared_mock.call_args.kwargs
    assert kwargs["backend"] == "brev"
    assert kwargs["label"] == "box"
    assert kwargs["mode"] == "docker"
    assert kwargs["ssh_ready_wait_seconds"] == cfg.ssh_ready_wait_seconds
    # Brev's own provisioning and teardown ride in as callables.
    assert callable(kwargs["provision"])
    assert callable(kwargs["teardown"])
    assert callable(kwargs["refresh_callback"])


def test_brev_teardown_runs_even_when_provisioning_fails(tmp_path):
    """Issue #29: a create that dies after allocating still leaks a billed box."""
    cfg = BrevConfig(auto_create_instances=True, mode="vm", on_finish="delete")
    app = _app(tmp_path, cfg=cfg)
    fn = _function(app, Image.from_registry("ubuntu:22.04"), module_file=_job_inside(tmp_path))
    ...
    with mock.patch.multiple(
        ...
        _instance_exists=mock.Mock(return_value=False),
        _refresh_ssh=mock.DEFAULT,
        _create_instance=mock.Mock(side_effect=RuntimeError("provider exploded")),
        _apply_on_finish=lambda *, instance, cfg: finished.append(instance),
    ):
        with pytest.raises(RuntimeError, match="provider exploded"):
            brev.run(app, fn, [], {}, instance="box")

    assert finished == ["box"], "teardown must still fire when provisioning fails"
