...
def test_check_preconditions_no_op_when_empty():
    ...
    with mock.patch("runplz.backends.ssh_common._ssh_capture") as ssh_mock:
        ssh_common._check_preconditions("box", {})
...
def test_check_preconditions_passes_when_observed_meets_minimum(capsys):
    ...
    with mock.patch("runplz.backends.ssh_common._ssh_capture", return_value=stdout):
        ssh_common._check_preconditions(
            "box",
...
def test_check_preconditions_warns_when_below_min_but_above_half(capsys):
    ...
    with mock.patch("runplz.backends.ssh_common._ssh_capture", return_value=stdout):
        ssh_common._check_preconditions("box", {"shm_gb": 4.0})
...
def test_check_preconditions_fails_when_below_half(capsys):
    ...
    with mock.patch("runplz.backends.ssh_common._ssh_capture", return_value=stdout):
        with pytest.raises(ssh_common.PreconditionFailed, match="shm_gb"):
            ssh_common._check_preconditions("box", {"shm_gb": 4.0})
...
def test_check_preconditions_gpu_count_below_half_fails():
    ...
    with mock.patch("runplz.backends.ssh_common._ssh_capture", return_value=stdout):
        with pytest.raises(ssh_common.PreconditionFailed, match="gpu_count"):
            ssh_common._check_preconditions("box", {"gpu_count": 4})
...
def test_check_preconditions_gpu_memory_translates_mib_to_gb(capsys):
    ...
    with mock.patch("runplz.backends.ssh_common._ssh_capture", return_value=stdout):
        with pytest.raises(ssh_common.PreconditionFailed, match="gpu_memory_gb"):
            ssh_common._check_preconditions("box", {"gpu_memory_gb": 80})
...
def test_check_preconditions_aggregates_multiple_failures():
    ...
    with mock.patch("runplz.backends.ssh_common._ssh_capture", return_value=stdout):
        with pytest.raises(ssh_common.PreconditionFailed) as ei:
            ssh_common._check_preconditions(
                "box",
...
def test_first_int_pulls_leading_number():
    assert ssh_common._first_int("  4096\n") == 4096
    assert ssh_common._first_int("") is None
    assert ssh_common._first_int(None) is None
    assert ssh_common._first_int("noise") is None


def test_bytes_to_gb_handles_zero_and_none():
    assert ssh_common._bytes_to_gb(None) is None
    assert ssh_common._bytes_to_gb(0) is None
    assert ssh_common._bytes_to_gb(2 * 1024**3) == pytest.approx(2.0)
